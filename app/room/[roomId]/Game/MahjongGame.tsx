...
const PIN_POS: Record<number, [number, number][]> = {
  ...
  2: [[0, -0.28], [0, 0.28]],
  3: [[0, -0.28], [0, 0], [0, 0.28]],
  4: [[-0.23, -0.24], [0.23, -0.24], [-0.23, 0.24], [0.23, 0.24]],
  5: [[-0.23, -0.28], [0.23, -0.28], [0, 0], [-0.23, 0.28], [0.23, 0.28]],
  6: [[-0.23, -0.28], [0.23, -0.28], [-0.23, 0], [0.23, 0], [-0.23, 0.28], [0.23, 0.28]],
  7: [[-0.23, -0.32], [0.23, -0.32], [-0.23, -0.08], [0.23, -0.08], [0, 0.14], [-0.23, 0.34], [0.23, 0.34]],
  8: [[-0.23, -0.32], [0.23, -0.32], [-0.23, -0.1], [0.23, -0.1], [-0.23, 0.12], [0.23, 0.12], [-0.23, 0.34], [0.23, 0.34]],
  9: [[-0.23, -0.32], [0.23, -0.32], [-0.23, -0.1], [0.23, -0.1], [0, 0], [-0.23, 0.12], [0.23, 0.12], [-0.23, 0.34], [0.23, 0.34]],
};
...
function drawMan(n: number, w: number, h: number, cx: number, cy: number): string {
...
function drawPin(n: number, w: number, h: number, cx: number, cy: number): string {
  const positions = PIN_POS[n];
  const spreadX = (w - 2) * 0.44, spreadY = (h - 2) * 0.44;
...
function drawSou(n: number, w: number, h: number, cx: number, cy: number): string {
  if (n === 1) {
    return `<text x="${cx}" y="${cy}" font-size="${Math.round(w * 0.55)}px" text-anchor="middle" dominant-baseline="central" fill="#2a7a30">竹</text>`;
  }
  const positions = PIN_POS[n];
  const spreadX = (w - 2) * 0.38, spreadY = (h - 2) * 0.44;
  const bw = Math.max(4, w * 0.13), bh = Math.max(7, h * 0.19);
...
function drawJi(n: number, w: number, h: number, cx: number, cy: number): string {
  const d = JI_DEFS[n - 1];
  const fs = Math.round(w * 0.50);
  if (!d.color) {
    const bx = cx - w * 0.22, by = cy - h * 0.22;
...
function getTileContent(tile: Tile | 'back', w: number, h: number, cx: number, cy: number): string {
  if (tile === 'back') {
    const ix = Math.round(w * 0.09), iy = Math.round(h * 0.07);
    const iw = w - 2 - ix * 2, ih = h - 2 - iy * 2;
...
function TileSVG({ tile, w, h, selected, drawn, noHover, onClick }: TileSVGProps) {
  const cls = [
    'tile-svg',
    selected ? 'selected' : '',
    drawn ? 'drawn-tile' : '',
...
function buildDeck(): Tile[] {
  const d: Tile[] = [];
  let id = 0;
  for (let n = 1; n <= 9; n++) for (let k = 0; k < 4; k++) d.push({ id: id++, suit: 'm', num: n });
  for (let n = 1; n <= 9; n++) for (let k = 0; k < 4; k++) d.push({ id: id++, suit: 'p', num: n });
  for (let n = 1; n <= 9; n++) for (let k = 0; k < 4; k++) d.push({ id: id++, suit: 's', num: n });
  for (let n = 1; n <= 7; n++) for (let k = 0; k < 4; k++) d.push({ id: id++, suit: 'z', num: n });
  return d;
}
...
function canFormSets(cnt: Record<string, number>): boolean {
  ...
  const k = keys.sort()[0];
  const suit = k[0], n = parseInt(k.slice(1));
  if (cnt[k] >= 3) {
    const r = { ...cnt }; r[k] -= 3; if (!r[k]) delete r[k];
  ...
  if (suit !== 'z') {
    ...
    if (cnt[k2] > 0 && cnt[k3] > 0) {
      const r = { ...cnt };
      r[k] -= 1; if (!r[k]) delete r[k];
      r[k2] -= 1; if (!r[k2]) delete r[k2];
      r[k3] -= 1; if (!r[k3]) delete r[k3];
      if (canFormSets(r)) return true;
    }
  }
  return false;
...
function isWinningHand(hand: Tile[]): boolean {
  ...
  hand.forEach(t => { const k = tk(t); cnt[k] = (cnt[k] || 0) + 1; });
  for (const pk of Object.keys(cnt)) {
    if (cnt[pk] >= 2) {
      const r = { ...cnt }; r[pk] -= 2; if (!r[pk]) delete r[pk];
      if (canFormSets(r)) return true;
    }
  }
...
function isTenpai(hand: Tile[]): boolean {
  ...
  const tries: Tile[] = [];
  (['m', 'p', 's'] as Suit[]).forEach(s => { for (let n = 1; n <= 9; n++) tries.push({ id: -1, suit: s, num: n }); });
...
function chooseDiscard(hand: Tile[]): number {
  ...
  hand.forEach((t, i) => {
    ...
    if (cnt[tk(t)] >= 2) s += 10;
    if (t.suit !== 'z') {
      [t.num - 1, t.num + 1].forEach(n => { if (cnt[t.suit + n]) s += 8; });
      [t.num - 2, t.num + 2].forEach(n => { if (cnt[t.suit + n]) s += 3; });
    }
    if (-s > bs) { bs = -s; best = i; }
  });
  return best;
...
export default function MahjongGame() {
  ...
  const [overlayData, setOverlayData] = useState({ title: '', detail: '', score: '', titleColor: '#ffd700' });
  ...
  const endGame = useCallback((type: string, winner: number, msg?: string) => {
    ...
    if (type === 'ryukyoku') {
      ...
      setOverlayData({ title: '流局', titleColor: '#aaccff', detail: '牌山がなくなりました\n' + (tenpai ? 'テンパイ！' : 'ノーテン'), score: '' });
    } else {
      const titleColor = winner === 0 ? '#ffd700' : '#ff6666';
      const title = winner === 0 ? (type === 'tsumo' ? 'ツモ！' : 'ロン！') : '和了られました';
      setOverlayData({ title, titleColor, detail: msg || '', score: `点数: ${gsRef.current.scores[0].toLocaleString()}点` });
    }
    setShowOverlay(true);
  }, []);

  // プレイヤーがツモで引く
  const playerDraw = useCallback((prevGs: GameState): GameState => {
    if (prevGs.deck.length === 0) {
      endGame('ryukyoku', -1);
      return { ...prevGs, gameOver: true, phase: 'end' };
    }
    const [drawn, ...deck] = prevGs.deck;
    const newGs = { ...prevGs, drawn: [drawn, ...prevGs.drawn.slice(1)], deck, phase: 'discard' as Phase };
  ...
  const runOpponentTurn = useCallback((playerIdx: number, prevGs: GameState) => {
    ...
    const doTurn = (player: number, currentGs: GameState) => {
      ...
      const newDiscards = gs1.discards.map((d, i) => i === player ? [...d, disc] : d);
  ...
  const discardSelected = useCallback(() => {
    setGs(prev => {
      if (prev.selectedTile === null || prev.phase !== 'discard' || prev.turn !== 0) return prev;
      ...
      if (prev.selectedTile === 100) {
        ...
        if (prev.drawn[0]) { newHand.push(prev.drawn[0]); newDrawn[0] = null; }
      }

      const newDiscards = prev.discards.map((d, i) => i === 0 ? [...d, tile] : d);
      const newHands = prev.hands.map((h, i) => i === 0 ? newHand : h);
      const next = { ...prev, hands: newHands, drawn: newDrawn, discards: newDiscards, selectedTile: null, turn: 1, phase: 'draw' as Phase };

      if (isTenpai(newHand)) setStatus('テンパイ！ロン待ち');
      if (next.deck.length === 0) { endGame('ryukyoku', -1); return { ...next, gameOver: true, phase: 'end' }; }
  ...
  const declareTsumo = useCallback(() => {
    setGs(prev => {
      ...
      const pts = calcPoints(1, false);
      const newScores = prev.scores.map((s, i) => i === 0 ? s + pts : s - Math.floor(pts / 3));
  ...
  const skipRon = useCallback(() => {
    setGs(prev => {
      ...
      const next = player + 1;
      const base = { ...prev, ronTile: null, ronFrom: null, phase: 'draw' as Phase };
      if (next > 3) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.key === 'Enter' || e.key === ' ') && gsRef.current.selectedTile !== null
        && gsRef.current.phase === 'discard' && gsRef.current.turn === 0) {
        discardSelected();
      }
    };
  ...
  // ツモ可能かチェック
  const canTsumo = gs.phase === 'discard' && gs.turn === 0 && gs.drawn[0] !== null &&
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#1a5c2a' }}>
        <div style={{ position: 'relative', width: 740, height: 610, background: '#1e7a34', border: '6px solid #0d3d19', borderRadius: 12, boxShadow: '0 0 60px rgba(0,0,0,0.6), inset 0 0 40px rgba(0,0,0,0.2)' }}>

          {/* 壁牌 */}
          <WallTiles />

          {/* 対面の手牌（裏） */}
          <div style={{ position: 'absolute', top: 30, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 2 }}>
            {Array.from({ length: gs.hands[2].length + (gs.drawn[2] ? 1 : 0) }).map((_, i) => (
              <div key={i} className="opp-tile" style={{ width: 20, height: 28 }} />
            ))}
          </div>
          {/* 上家の手牌（裏） */}
          <div style={{ position: 'absolute', left: 28, top: '50%', transform: 'translateY(-60%)', display: 'flex', flexDirection: 'column', gap: 2 }}>
            {Array.from({ length: gs.hands[1].length + (gs.drawn[1] ? 1 : 0) }).map((_, i) => (
              <div key={i} className="opp-tile" style={{ width: 28, height: 20 }} />
            ))}
          </div>
          {/* 下家の手牌（裏） */}
          <div style={{ position: 'absolute', right: 28, top: '50%', transform: 'translateY(-60%)', display: 'flex', flexDirection: 'column', gap: 2 }}>
            {Array.from({ length: gs.hands[3].length + (gs.drawn[3] ? 1 : 0) }).map((_, i) => (
              <div key={i} className="opp-tile" style={{ width: 28, height: 20 }} />
            ))}
          </div>

          {/* 河（捨て牌） */}
          <DiscardPile id="top"    tiles={gs.discards[2]} style={{ position: 'absolute', bottom: 'calc(52% + 81px)', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 2, flexWrap: 'wrap-reverse', width: 190, justifyContent: 'center', alignItems: 'flex-end' }} />
          <DiscardPile id="left"   tiles={gs.discards[1]} style={{ position: 'absolute', right: 'calc(50% + 98px)', top: '52%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 152, flexWrap: 'wrap', alignItems: 'flex-end' }} />
          <DiscardPile id="right"  tiles={gs.discards[3]} style={{ position: 'absolute', left: 'calc(50% + 98px)', top: '52%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 152, flexWrap: 'wrap' }} />
          <DiscardPile id="player" tiles={gs.discards[0]} style={{ position: 'absolute', top: 'calc(52% + 80px)', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 2, flexWrap: 'wrap', width: 190, justifyContent: 'center' }} />

          {/* 中央エリア */}
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -52%)', width: 190, height: 155, background: 'rgba(0,0,0,0.18)', borderRadius: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, zIndex: 1 }}>
            <div style={{ fontSize: 26, fontWeight: 900, color: '#ffd700', textShadow: '0 0 10px rgba(255,215,0,.5)' }}>東</div>
            <div style={{ fontSize: 13, color: '#aad' }}>東1局</div>
            <div style={{ fontSize: 11, color: '#ccc' }}>残り牌: {gs.deck.length}</div>
          </div>

          {/* スコア */}
          <div style={{ position: 'absolute', top: 8, left: 8, background: 'rgba(0,0,0,.5)', borderRadius: 6, padding: '5px 9px', fontSize: 11, color: '#ddd', lineHeight: 1.7 }}>
          ...
          {/* プレイヤー手牌 */}
          <div style={{ position: 'absolute', bottom: 50, left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'row', alignItems: 'flex-end', gap: 0 }}>
            <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end' }}>
              {gs.hands[0].map((tile, idx) => (
                <TileSVG key={tile.id} tile={tile} w={46} h={62}
                  selected={gs.selectedTile === idx}
                  onClick={() => gs.selectedTile === idx ? discardSelected() : selectTile(idx)}
                />
              ))}
            </div>
            {/* ツモ牌 */}
            {gs.drawn[0] && (
              <>
                ...
                <TileSVG tile={gs.drawn[0]} w={46} h={62}
                  drawn
                  selected={gs.selectedTile === 100}
                  onClick={() => gs.selectedTile === 100 ? discardSelected() : selectTile(100)}
                />
              </>
            )}
            {/* 副露牌 */}
            {gs.melds[0].length > 0 && (
              <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end', marginLeft: 10 }}>
                {gs.melds[0].map((meld, mi) =>
                  meld.tiles.map((t, ti) => <TileSVG key={`${mi}-${ti}`} tile={t} w={36} h={48} noHover />)
                )}
              </div>
            )}
          </div>
          ...
          <div style={{ position: 'absolute', bottom: 10, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 5 }}>
            {canRon && (
              <button className="action-btn" style={{ borderColor: '#888', color: '#ccc' }} onClick={skipRon}>スキップ</button>
            )}
            <button className="action-btn" disabled>カン</button>
            <button className="action-btn" disabled>チー</button>
            <button className="action-btn" disabled>ポン</button>
            <button className={`action-btn${canRon ? ' highlight' : ''}`} disabled={!canRon} onClick={declareRon}>ロン</button>
            <button className={`action-btn${canTsumo ? ' highlight' : ''}`} disabled={!canTsumo} onClick={declareTsumo}>ツモ</button>
          </div>

          {/* ステータス */}
          <div style={{ position: 'absolute', bottom: 118, left: '50%', transform: 'translateX(-50%)', fontSize: 11, color: '#aaffaa', whiteSpace: 'nowrap' }}>
            {status}
          </div>

          {/* 通知 */}
          {notification && (
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'rgba(0,0,0,.85)', border: '2px solid #ffd700', borderRadius: 8, padding: '12px 24px', fontSize: 20, fontWeight: 700, color: '#ffd700', zIndex: 50, pointerEvents: 'none' }}>
              {notification}
            </div>
          )}
          ...
          {/* ゲーム終了オーバーレイ */}
          {showOverlay && (
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.75)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: 8, zIndex: 100 }}>
              <div style={{ fontSize: 44, fontWeight: 900, color: overlayData.titleColor, textShadow: '0 0 20px rgba(255,215,0,.8)', marginBottom: 12 }}>{overlayData.title}</div>
              <div style={{ fontSize: 16, color: '#fff', marginBottom: 8, textAlign: 'center', maxWidth: 400, lineHeight: 1.8, whiteSpace: 'pre-line' }}>{overlayData.detail}</div>
              {overlayData.score && <div style={{ fontSize: 20, fontWeight: 700, color: '#88ff88', marginBottom: 20 }}>{overlayData.score}</div>}
              <button
                style={{ padding: '12px 32px', background: 'linear-gradient(145deg,#8a6030,#4a3010)', border: '2px solid #ffd700', borderRadius: 8, color: '#ffd700', fontSize: 16, fontWeight: 700, fontFamily: 'Noto Sans JP, sans-serif', cursor: 'pointer' }}
                onClick={initGame}
              >もう一局</button>
            </div>
          )}
...
function WallTiles() {
  return (
    <>
      <div style={{ position: 'absolute', top: 8, left: 170, right: 22, height: 18, display: 'flex', gap: 1 }}>
        {Array.from({ length: 22 }).map((_, i) => <div key={i} className="wall-tile" style={{ width: 15, height: 18 }} />)}
      </div>
      <div style={{ position: 'absolute', bottom: 110, left: 55, right: 55, height: 16, display: 'flex', gap: 1 }}>
        {Array.from({ length: 22 }).map((_, i) => <div key={i} className="wall-tile" style={{ width: 15, height: 16 }} />)}
      </div>
      <div style={{ position: 'absolute', top: 52, left: 8, bottom: 115, width: 16, display: 'flex', flexDirection: 'column', gap: 1 }}>
        {Array.from({ length: 18 }).map((_, i) => <div key={i} className="wall-tile" style={{ width: 16, height: 14 }} />)}
      </div>
      <div style={{ position: 'absolute', top: 52, right: 8, bottom: 115, width: 16, display: 'flex', flexDirection: 'column', gap: 1 }}>
        {Array.from({ length: 18 }).map((_, i) => <div key={i} className="wall-tile" style={{ width: 16, height: 14 }} />)}
      </div>
    </>
  );
...
function DiscardPile({ tiles, style }: { id: string; tiles: Tile[]; style: React.CSSProperties }) {
  return (
    <div style={style}>
      {tiles.map((t, i) => <TileSVG key={`${t.id}-${i}`} tile={t} w={26} h={35} noHover />)}
    </div>
  );
}
